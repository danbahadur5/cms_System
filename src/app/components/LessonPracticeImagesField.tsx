...
export function LessonPracticeImagesField({
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const imageFiles = Array.from(files).filter(looksLikeImageFile);
    if (imageFiles.length < files.length) {
      toast.warn('Only images are allowed here. Use the attachments field for other files.');
    }
    if (imageFiles.length > 0) enqueueFiles(imageFiles);
  };
      ...
      {(images.length > 0 || pendingSlots.length > 0) && (
        <div className="mt-3 rounded-lg border border-gray-200 bg-gray-50/50 p-3">
          <div className="mb-2 flex items-center justify-between">
            <h5 className="text-xs font-semibold text-gray-700">Media Preview ({images.length} uploaded)</h5>
            {uploading && (
              <span className="flex items-center gap-1.5 text-[10px] font-medium text-blue-600">
                <Loader2 className="h-3 w-3 animate-spin" />
                Uploading {pendingSlots.length} more...
              </span>
            )}
          </div>
