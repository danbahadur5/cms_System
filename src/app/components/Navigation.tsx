...
export function Navigation() {
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 w-full border-b bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            ...
            <button
              type="button"
              className="md:hidden inline-flex items-center justify-center p-2 -mr-2 rounded-md hover:bg-gray-100 transition-colors"
      ...
      {/* Mobile Navigation — overlay with backdrop (outside <nav> to avoid stacking context issues) */}
      {mobileMenuOpen && (
        <>
          {/* Backdrop */}
          <div
          ...
          />
          {/* Menu panel */}
          <div className="fixed inset-x-0 top-16 bottom-0 z-50 md:hidden bg-white overflow-y-auto animate-in slide-in-from-top-4 duration-200">
            <div className="px-4 py-4 space-y-1">
              <Link
                to="/"
                className="flex items-center gap-3 px-3 py-2.5 text-base font-medium text-gray-700 hover:bg-blue-50 hover:text-blue-600 rounded-md transition-colors"
                onClick={() => setMobileMenuOpen(false)}
              >
                Home
              </Link>
              ...
              <div className="space-y-1">
                <button
                  type="button"
                  className="w-full flex items-center justify-between gap-3 px-3 py-2.5 text-base font-medium text-gray-700 hover:bg-blue-50 hover:text-blue-600 rounded-md transition-colors"
                  ...
                  <div className="pl-4 pr-2 pb-2 space-y-1 animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    {courses.map((course) => (
                      <Link
                        ...
                        className="flex flex-col px-3 py-2.5 text-sm text-gray-600 hover:bg-gray-50 rounded-md transition-colors border-l-2 border-transparent hover:border-blue-200 ml-1"
                        onClick={() => setMobileMenuOpen(false)}
                      >
              ...
              <div className="pt-6 mt-4 border-t border-gray-100">
                {adminEmail ? (
                  <>
                    ...
                    <button
                      type="button"
                      className="w-full flex items-center gap-3 px-3 py-2.5 text-base font-medium text-red-600 hover:bg-red-50 rounded-md transition-colors"
                      onClick={handleStaffSignOut}
                    >
                  ...
                  <Link
                    to="/admin/login"
                    className="flex items-center gap-3 px-3 py-2.5 text-base font-medium text-gray-600 hover:bg-gray-50 rounded-md transition-colors"
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    Staff sign in
                  </Link>
